...
Webhook chat is deliberately read-only: Goose receives repository metadata, README, Issue/PR text, bounded diffs, recent discussion, and the host-curated repository knowledge cache when R2 is configured, but no repository tools or credentials. It cannot edit code, run commands, push commits, or execute `/recheck` and `/conflict`; an exact `/recheck` or `/conflict` command receives a guidance reply pointing to Action mode instead of silence. The webhook service also runs the same automatic issue and pull-request triage as Action mode on `opened`, `edited`, and `reopened` events for App-installed repositories; set `WEBHOOK_TRIAGE_ENABLED=false` to disable it there. All webhook-mode local writes stay inside the current working directory: the per-repository knowledge cache lives at `.ghbot-tmp/webhook-knowledge/<repository id>/`. Replies follow the language of the latest comment and include no provider or GitHub secrets. Leaving `WEBHOOK_ENABLED` unset preserves the normal Action-only deployment.
